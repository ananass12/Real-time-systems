...
#define NUM_SAMPLES 5000   //5000*2мс ≈ 10 секунд
...
int main(void) {
    ...
    const int64_t period_ns = 2 * MILLION; //2мс
    ...
    for (samples = 0; samples < NUM_SAMPLES; ++samples) {
        ...
        /* TIMER_ABSTIME означает: "спать ДО этого момента", а не "спать ЕЩЁ N нс".
    ...
    //первые несколько измерений для наглядности
...
int main(void) {
    ...
    const long period_ns = 2 * 1000000L; 
    struct timespec req;
